...
class MCPServer:
    ...
    async def call(self, name, args, allow_dangerous=False):
        return await self.tools.call(name, args, allow_dangerous)
                    ...
                    "content": [
                        {
                            ...
                            "text": json.dumps(
                                await self.call(
                                    ...
                                    bool(params.get("allow_dangerous", False)),
                                ),
                                default=str,
                            ),
                        }
...
async def mount_mcp_stdio(registry: ToolRegistry, client: MCPStdioClient, prefix="mcp"):
    for schema in await client.list_tools():
        ...
        name = fn.get("name")
        desc = fn.get("description", "MCP tool")
        params = fn.get("parameters", {})

        async def invoke(args, n=name):
            return await client.call_tool(n, args)
