...
def default_tools(root:Path|None=None):
 ...
 async def read_file(a):
  p=safe_path(a["path"])
  max_bytes=max(1,min(int(a.get("max_bytes",1024*1024)),8*1024*1024))
  data=p.read_bytes()
  truncated=len(data)>max_bytes
  content=data[:max_bytes].decode("utf-8",errors="replace")
  return {"path":str(p),"content":content,"bytes":len(data),"truncated":truncated}
 r.register(ToolSpec("read_file","Read a UTF-8 text file in the workspace.",{"type":"object","properties":{"path":{"type":"string"},"max_bytes":{"type":"integer","minimum":1,"maximum":8388608}},"required":["path"]},read_file))
 ...
 async def search_files(a):
  base=safe_path(a.get("path",".")); pattern=str(a.get("pattern","")); needle_text=str(a.get("query","")); max_results=max(1,min(int(a.get("max_results",50)),200))
  max_file_bytes=max(1,min(int(a.get("max_file_bytes",1024*1024)),8*1024*1024))
  matches=[]
  for item in base.rglob(pattern or "*"):
   ...
   if needle_text:
    try:
     if needle_text.lower() not in item.read_bytes()[:max_file_bytes].decode("utf-8",errors="replace").lower():
      continue
    except OSError:
     continue
 ...
 r.register(ToolSpec("search_files","Find workspace files, optionally filtering file contents; read-only.",{"type":"object","properties":{"path":{"type":"string"},"pattern":{"type":"string"},"query":{"type":"string"},"max_results":{"type":"integer","minimum":1,"maximum":200},"max_file_bytes":{"type":"integer","minimum":1,"maximum":8388608}}},search_files))
 ...
 async def shell(a):
  timeout=max(1,min(int(a.get("timeout",20)),60))
  try:
   cp=subprocess.run(a["command"],shell=True,capture_output=True,text=True,timeout=timeout,cwd=str(root))
   return {"returncode":cp.returncode,"stdout":cp.stdout[-12000:],"stderr":cp.stderr[-12000:],"timed_out":False}
  except subprocess.TimeoutExpired as e:
   return {"returncode":None,"stdout":(e.stdout or "")[-12000:],"stderr":(e.stderr or "")[-12000:],"timed_out":True,"timeout":timeout}
 r.register(ToolSpec("shell","Run a shell command in the workspace; requires explicit approval.",{"type":"object","properties":{"command":{"type":"string"},"timeout":{"type":"integer","minimum":1,"maximum":60}},"required":["command"]},shell,True))
 return r
class ToolExecutor:
