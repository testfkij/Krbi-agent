...
class Handler(BaseHTTPRequestHandler):
    ...
    def settings_html() -> str:
        ...
        return (
            ...
            f'<div><b>Banner</b><div style="margin-top:8px;display:flex;gap:6px"><input id="banner_input" class="field" value="{html.escape(SETTINGS.banner_text, quote=True)}" maxlength="120"><button onclick="setSetting(\'banner\',document.getElementById(\'banner_input\').value)">Save</button></div></div>'
    ...
    def do_POST(self) -> None:
        ...
        if path == "/settings":
            ...
            if action == "approve" and value:
                ...
                SETTINGS.banner_text = value[:120] or "KRBI // AGENT"
            else:
        ...
        async def run() -> None:
            async for event in AGENT.run(session.chat_id, provider, model, prompt, system_prompt=session.system, goal=session.goal, api_key=session.api_key):
                if event.type == "delta":
                    ...
                    self.wfile.write(("data: Tool running: " + event.message + "\n\n").encode()); self.wfile.flush()
                elif event.type == "tool_result":
                    self.wfile.write(("data: Tool completed: " + event.message + "\n\n").encode()); self.wfile.flush()
